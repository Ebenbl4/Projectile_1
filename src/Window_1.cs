...
namespace Medic
{
    ...
    {
        private Image clueImage_1;
        private bool isDrawingMode = false;
        private bool isDrawingCircleMode = false;
        ...
        public Window_1()
        {
            InitializeComponent();
            LoadClueImage();
            btn_switch_1.Enabled = false;
            btn_switch_2.Enabled = false;
            ...
            btn_measure_1_2.Enabled = true;
            btn_measure_1_1.Enabled = true;
        }
        ...
        private void LoadClueImage()
        {
            try
            {
                string clueImagePath = Path.Combine(Application.StartupPath, "clue_1.png");

                if (File.Exists(clueImagePath))
                {
                    clueImage_1 = Image.FromFile(clueImagePath);
                }
                else
                {
                    MessageBox.Show("Файл clue_1.png не найден в папке с программой!",
                                  "Предупреждение",
            ...
            {
                MessageBox.Show($"Ошибка при загрузке clue_1.png: {ex.Message}",
                              "Ошибка",
                              MessageBoxButtons.OK,
        ...
        {
            if (textBox_1_3_ISh.Text == "")
            {
                MessageBox.Show("Индекс ISh не измерен!",
                                      "Ошибка",
                                      MessageBoxButtons.OK,
                                      MessageBoxIcon.Error);
            }
            ...
            {
                if (SharedData.ISh >= 0.6 && SharedData.ISh <= 1.0)
                {
                ...
                {
                    MessageBox.Show("Индекс сферичности головки ISh выходит за пределы требуемого диапазона.\r\nПрименение программы невозможно",
                      "Ошибка",
                      MessageBoxButtons.OK,
                      MessageBoxIcon.Error);
                }
        ...
        private void btn_select_1_Click(object sender, EventArgs e)
        {
            using (OpenFileDialog openFileDialog = new OpenFileDialog())
            {
                ...
                if (openFileDialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        SharedData.Radiograph_1 = (Bitmap)Image.FromFile(openFileDialog.FileName);
                        pictureBox1.Image = SharedData.Radiograph_1;

                        btn_switch_1.Enabled = true;
                        btn_switch_2.Enabled = (clueImage_1 != null);
                    }
                    catch (Exception ex)
                    {
        ...
        private void btn_switch_1_Click(object sender, EventArgs e)
        {
            if (SharedData.Radiograph_1 != null)
            {
                pictureBox1.Image = SharedData.Radiograph_1;
            }
        }
        ...
        private void btn_switch_2_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Файл - шпаргалка clue_1.png не найден!",
                      "Ошибка",
                      MessageBoxButtons.OK,
                      MessageBoxIcon.Error);
            }
        ...
        private void btn_measure_1_1_Click(object sender, EventArgs e)
        {
            if (SharedData.Radiograph_1 != null)
            {
                if (pictureBox1.Image != SharedData.Radiograph_1)
                {
        ...
        private void PictureBox1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (isDrawingCircleMode)
                textBox_1_1_Dh.Text = Math.Round(length, 2).ToString();
            else
                textBox_1_2_R.Text = Math.Round(length, 2).ToString();
        ...
        private void PictureBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (drawingLayer != null && pictureBox1.Image != null)
            {
                ...
                int offsetX = (int)((pictureBox1.Width - pictureBox1.Image.Width * scale) / 2);
                int offsetY = (int)((pictureBox1.Height - pictureBox1.Image.Height * scale) / 2);

                // Отрисовка слоя с фигурой
                e.Graphics.DrawImage(drawingLayer, offsetX, offsetY,
                    pictureBox1.Image.Width * scale, pictureBox1.Image.Height * scale);
            }
        ...
        private void CreateFinalLine(Point secondPoint)
        {
            ...
            drawingLayer = new Bitmap(pictureBox1.Image.Width, pictureBox1.Image.Height);
        ...
        private void CreateFinalCircle(Point secondPoint)
        {
            ...
            textBox_1_1_Dh.Text = Math.Round(diameter, 2).ToString();
        ...
        private Point GetImageCoordinates(Point pictureBoxPoint)
        {
            ...
            imageX = Math.Max(0, Math.Min(imageX, pictureBox1.Image.Width - 1));
            imageY = Math.Max(0, Math.Min(imageY, pictureBox1.Image.Height - 1));
        ...
        private void btn_measure_1_2_Click(object sender, EventArgs e)
        {
            if (SharedData.Radiograph_1 != null)
            {
                if (pictureBox1.Image != SharedData.Radiograph_1)
                ...
                {
                    // Сброс состояния и включение режима рисования окружности
                    isDrawingMode = true;
                    isDrawingCircleMode = true;
                    isFirstPointSet = false;
                    textBox_1_1_Dh.Text = "0";
